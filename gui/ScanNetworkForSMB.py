        ...
        while self.pending_count > 0 and (timeout == 0 or time.time() < end_time):
            ...
            try:
                ...
                try:
                    ...
                    self.pending_count -= 1
                    for _curr_data in ret:
                        if _curr_data[1] == 32:
                            # Hostname
                            _hostname =  _curr_data[0]
                        if _curr_data[1] == 30:
                            # Domain
                            _domain =  _curr_data[0]
                    print("Found " + _hostname + '(' + ip + '), domain/workgroup = ' + _domain)

                    self.hosts.append([_hostname, ip, _domain])
                ...
                if type(ex) is types.TupleType:
                    ...
                    raise ex
    def list_all_hosts(self, _subnet = '192.168.0',_timeout=3):
        start_ip = DottedIPToInt(_subnet + '.0')
        end_ip = DottedIPToInt(_subnet + '.256')
        for ip in range(start_ip, end_ip):
            self.queryIPForName(IntToDottedIP(ip))
            self.poll()
